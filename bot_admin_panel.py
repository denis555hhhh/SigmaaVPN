...
class BotAdminPanel:
    ...
    def create_ui(self):
        ...
        file_menu.add_command(label="Открыть локальную БД", command=self.open_db)
        file_menu.add_command(label="Подключиться к Railway", command=self.connect_railway)
        ...
        self.status_label = tk.Label(top_frame, text="📁 Локальная БД", 
                                    font=("Arial", 10), bg=self.bg_color, fg='#00aa00')
    ...
    def connect_railway(self):
        ...
        dialog.title("Подключение к Railway")
        dialog.geometry("400x200")
        dialog.configure(bg=self.bg_color)
        
        tk.Label(dialog, text="Введите данные для подключения к Railway:", 
                bg=self.bg_color, fg=self.fg_color).pack(pady=10)
        
        tk.Label(dialog, text="URL БД (например: postgresql://...):", 
                bg=self.bg_color, fg=self.fg_color).pack()
        url_entry = tk.Entry(dialog, width=50)
        url_entry.pack(pady=5)
        
        tk.Label(dialog, text="Или скачайте БД через Shell:", 
                bg=self.bg_color, fg=self.fg_color).pack(pady=10)
        
        tk.Label(dialog, text="1. Откройте Railway Dashboard", 
                bg=self.bg_color, fg=self.fg_color).pack()
        tk.Label(dialog, text="2. Откройте Shell сервиса", 
                bg=self.bg_color, fg=self.fg_color).pack()
        tk.Label(dialog, text="3. Выполните: cat sigmavpn.db > /tmp/db.db", 
                bg=self.bg_color, fg=self.fg_color).pack()
        tk.Label(dialog, text="4. Скачайте файл", 
                bg=self.bg_color, fg=self.fg_color).pack()
        
        def connect():
            url = url_entry.get()
            if url:
                self.railway_url = url
                self.use_railway = True
                self.status_label.config(text="☁️ Railway БД", fg=self.accent_color)
                messagebox.showinfo("Успех", "Подключено к Railway!")
                dialog.destroy()
            else:
                messagebox.showwarning("Ошибка", "Введите URL БД")
        
        tk.Button(dialog, text="Подключиться", command=connect, 
                 bg=self.accent_color, fg='black').pack(pady=10)
...
Дата: {datetime.now().strftime('%d.%m.%Y %H:%M:%S')}
    ...
    def open_db(self):
        ...
        if file:
            self.db_path = file
            self.use_railway = False
            self.status_label.config(text="📁 Локальная БД", fg='#00aa00')
            self.load_data()
